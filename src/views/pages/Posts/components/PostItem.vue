<template>
  ...
  >
    <div v-if="postAuthor" class="flex justify-between items-center px-4 py-2 border-b">
      <!-- header -->
    ...
    <router-link :to="`/post/${post.id}`" v-if="post.photoURL" class="w-fit">
      <!-- image -->
      <img
        v-if="invalidURL"
    ...
    <div class="p-5 pb-2.5">
      <router-link :to="`/post/${post.id}`">
        <h5
          v-if="post.title"
          class="mb-2 text-2xl font-bold tracking-tight text-gray-900 break-word"
        >
          {{ post.title }}
        </h5>
      </router-link>
      <p
        ...
        class="mb-3 font-normal text-gray-700 line-clamp_3 block break-word"
      >
        {{ post.description }}
      </p>
